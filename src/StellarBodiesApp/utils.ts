...
export function computeEllipseXPosition(d: StellarBody, containerCenterX: number, sunRadius: number, angleValue: number): number {
    let r = computeRadius(d, sunRadius);

    return containerCenterX + r * Math.cos(angleValue);
}

export function computeEllipseYPosition(d: StellarBody, containerCenterY: number, sunRadius: number, angleValue: number): number {
    let r = computeRadius(d, sunRadius);
...
export function computeStellarObjectRadius(meanRadius: number): number {
    return Math.floor(meanRadius / ViewConfig.scale) === 0 ? ViewConfig.minimalPlanetSize : Math.floor(meanRadius / ViewConfig.scale);
}
